...
class MsgHandler(blcsdk.BaseHandler):
    ...
    def _on_add_text(self, client: cli.BlcPluginClient, message: models.AddTextMsg, extra: models.ExtraData):
        ...
        if self.options and self.options.url:
            try:
                response = requests.post(self.options.url, json={'content': message.content})
                if response.status_code == 200:
                    print('弹幕发送成功')
                else:
                    print(f'弹幕发送失败: {response.status_code} - {response.text}')
            except requests.RequestException as e:
                print(f'弹幕发送异常: {e}')
        else:
            print('未配置服务器地址，无法发送弹幕')
...
def opt_parser():
    ...
    parser.add_argument('--url', type=str, default='http://localhost:83/', help='需要将弹幕发送到的服务器地址')
...
async def main(options: Optional[argparse.Namespace] = None) -> int:
    ...
    print('blivechat plugin init success.')
